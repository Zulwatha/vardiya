...
`reclaimStale` puts abandoned active jobs (stale heartbeat) back to `pending`.
`release` does the same for a single id on graceful shutdown, and also undoes
the claim's attempt increment so a clean stop does not burn a retry.
